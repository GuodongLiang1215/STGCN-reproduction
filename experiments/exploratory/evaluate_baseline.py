...
test_normalized = scaler.transform(test_data).astype(np.float32)

x_test, y_test = dataloader.data_transform(test_normalized, N_HIS, N_PRED, device)
...
with torch.no_grad():
    for x_batch, y_batch in test_loader:
        prediction_batch = model(x_batch).view(len(x_batch), n_vertex)

        predictions_normalized.append(prediction_batch.cpu().numpy())

        targets_normalized.append(y_batch.cpu().numpy())

predictions_normalized = np.concatenate(predictions_normalized, axis=0)

targets_normalized = np.concatenate(targets_normalized, axis=0)

# 恢复成真实交通速度
predictions = scaler.inverse_transform(predictions_normalized)

targets = scaler.inverse_transform(targets_normalized)
...
mape = np.mean(absolute_error / np.maximum(np.abs(targets), epsilon)) * 100

wmape = (absolute_error.sum() / np.maximum(np.abs(targets).sum(), epsilon)) * 100
...
median_sensor = int(sorted_sensor_indices[len(sorted_sensor_indices) // 2])

print("\n" + "=" * 65)
print("3. 每个传感器的表现")
print("=" * 65)

print(f"表现最好传感器: {best_sensor}, " f"MAE={sensor_mae[best_sensor]:.4f}")

print(f"中等表现传感器: {median_sensor}, " f"MAE={sensor_mae[median_sensor]:.4f}")

print(f"表现最差传感器: {worst_sensor}, " f"MAE={sensor_mae[worst_sensor]:.4f}")


metric_table = pd.DataFrame(
    {
        "Metric": ["MAE", "RMSE", "MAPE_percent", "WMAPE_percent"],
        "Value": [mae, rmse, mape, wmape],
    }
)

metric_table.to_csv("results_15min_metrics.csv", index=False)
...
plot_table = pd.DataFrame(
    {
        ...
        "true_speed": targets[:plot_length, median_sensor],
        "predicted_speed": predictions[:plot_length, median_sensor],
    }
)

plot_table.to_csv("results_15min_predictions.csv", index=False)

plt.figure(figsize=(12, 5))

plt.plot(plot_table["time_step"], plot_table["true_speed"], label="True speed")

plt.plot(
    plot_table["time_step"], plot_table["predicted_speed"], label="Predicted speed"
)

plt.xlabel("5-minute time step")
plt.ylabel("Traffic speed")

plt.title(f"STGCN 15-minute prediction — " f"Sensor {median_sensor}")

plt.legend()
plt.tight_layout()

plt.savefig("results_15min_prediction.png", dpi=200)
