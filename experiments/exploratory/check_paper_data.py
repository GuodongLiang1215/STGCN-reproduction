...
def make_daily_sequences(data: np.ndarray, start_day: int, num_days: int) -> np.ndarray:
    """
    按天制作 STGCN 序列。
    ...
    sequences = np.empty((total_samples, N_FRAME, N_ROUTE, 1), dtype=np.float32)
    ...
    for day in range(start_day, start_day + num_days):
        ...
        for slot in range(slots_per_day):
            ...
            sequences[sample_index, :, :, 0] = data[start:end]
...
raw_data = pd.read_csv(DATA_PATH, header=None, dtype=np.float32).to_numpy()
...
train_sequences = make_daily_sequences(raw_data, start_day=0, num_days=N_TRAIN_DAYS)

val_sequences = make_daily_sequences(
    raw_data, start_day=N_TRAIN_DAYS, num_days=N_VAL_DAYS
)

test_sequences = make_daily_sequences(
    raw_data, start_day=N_TRAIN_DAYS + N_VAL_DAYS, num_days=N_TEST_DAYS
)
...
x_train_tensor = torch.from_numpy(np.ascontiguousarray(x_train))

y_train_tensor = torch.from_numpy(np.ascontiguousarray(y_train))
...
print("每天样本数:", f"{DAY_SLOT} - {N_FRAME} + 1 = {slots_per_day}")
...
print("标准化后训练序列平均值:", float(train_sequences.mean()))

print("标准化后训练序列标准差:", float(train_sequences.std()))
...
print("第 1 天最后一个窗口:")
print(f"开始索引={last_start_day_0}, " f"目标索引={last_target_day_0}")

print("第 2 天第一个窗口:")
print(f"开始索引={first_start_day_1}, " f"目标索引={first_target_day_1}")
