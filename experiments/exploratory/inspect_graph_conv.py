...
gso_sparse = utility.calc_gso(adj, GSO_TYPE)

# Chebyshev图卷积需要进一步缩放GSO
gso_cheb_sparse = utility.calc_chebynet_gso(gso_sparse)
...
x = torch.randn(BATCH_SIZE, C_IN, TIME_LENGTH, n_vertex, device=device)
...
graph_conv = GraphConvLayer(
    graph_conv_type=GRAPH_CONV_TYPE, c_in=C_IN, c_out=C_OUT, Ks=KS, gso=gso, bias=True
).to(device)
...
with torch.no_grad():
    ...
    # T1：经过一次图传播
    x_1 = torch.einsum("hi,btij->bthj", gso, x_permuted)

    # T2：Chebyshev递推
    x_2 = torch.einsum("hi,btij->bthj", 2 * gso, x_1) - x_0

    cheb_features = torch.stack([x_0, x_1, x_2], dim=2)

    # 图卷积内部原始输出
    raw_graph_output = graph_conv.cheb_graph_conv(x_aligned)
...
print("图卷积权重形状:", graph_conv.cheb_graph_conv.weight.shape)

print("图卷积内部输出形状:", raw_graph_output.shape)

print("完整图卷积层输出形状:", output.shape)
