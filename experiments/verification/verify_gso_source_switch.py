...
def main():
    ...
    for gso_source in ("current", "original"):
        ...
        isolated_indices = graph_info["isolated_node_indices"]

        isolated_diagonal = torch.diag(gso)[isolated_indices]

        print("Isolated-node diagonal values:", isolated_diagonal.tolist())
        print()
    ...
    assert original["graph_info"]["nonzero_count"] == 1664

    assert original["graph_info"]["isolated_node_count"] == 3

    isolated_indices = original["graph_info"]["isolated_node_indices"]

    assert isolated_indices == [13, 135, 226]

    current_isolated_diagonal = torch.diag(current["gso"])[isolated_indices]

    original_isolated_diagonal = torch.diag(original["gso"])[isolated_indices]

    assert torch.allclose(
        current_isolated_diagonal,
        torch.zeros_like(current_isolated_diagonal),
        atol=1e-6,
        rtol=0.0,
    )
    ...
    assert torch.allclose(
        ...
        -torch.ones_like(original_isolated_diagonal),
        atol=1e-6,
        rtol=0.0,
    )

    gso_max_abs_difference = torch.max(
        torch.abs(current["gso"] - original["gso"])
    ).item()

    print("=" * 72)
    print("Comparison")
    print("=" * 72)
    print("Maximum absolute GSO difference:", gso_max_abs_difference)
    print("GSO exactly equal:", torch.equal(current["gso"], original["gso"]))
