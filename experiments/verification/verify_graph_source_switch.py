...
def main():
    ...
    for graph_source in ("current", "official"):
        gso, n_vertex, graph_info = prepare_graph(
            dataset="pemsd7-m", graph_source=graph_source, device=device
        )
        ...
        print(
            "Adjacency non-zero diagonal values:", graph_info["diagonal_nonzero_count"]
        )
    ...
    assert official["graph_info"]["nonzero_count"] == 1664

    assert official["graph_info"]["diagonal_nonzero_count"] == 0

    gso_max_abs_difference = torch.max(
        torch.abs(current["gso"] - official["gso"])
    ).item()

    print("=" * 72)
    print("Comparison")
    print("=" * 72)
    print("Current adjacency non-zero values:", current["graph_info"]["nonzero_count"])
    print(
        "Official adjacency non-zero values:", official["graph_info"]["nonzero_count"]
    )
    print("Maximum absolute GSO difference:", gso_max_abs_difference)
    print("GSO exactly equal:", torch.equal(current["gso"], official["gso"]))

    if gso_max_abs_difference == 0:
        raise AssertionError(
            "The two graph sources unexpectedly produced " "identical GSOs."
        )
