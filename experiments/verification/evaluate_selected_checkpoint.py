...
# Import PyTorch before pandas on Windows to avoid DLL load-order issues.
import torch
...
from paper_main import evaluate_autoregressive, prepare_graph, print_metrics
...
def main():
    ...
    selection_directory = result_directory / "selection_protocol"
    ...
    (gso, n_vertex, graph_info) = prepare_graph(
        dataset=dataset, graph_source=graph_source, device=device, gso_source=gso_source
    )

    print("\nGraph:")
    print("Adjacency non-zero values:", graph_info["nonzero_count"])
    print("Isolated-node indices:", graph_info["isolated_node_indices"])
    print("Largest eigenvalue:", graph_info["lambda_max"])

    (scaler, _x_train, _y_train, _val_sequences, test_sequences) = (
    ...
    if abs(float(scaler.mean) - float(checkpoint["scaler_mean"])) > 1e-8:
        raise ValueError("Scaler mean does not match the checkpoint.")

    if abs(float(scaler.std) - float(checkpoint["scaler_std"])) > 1e-8:
        raise ValueError("Scaler std does not match the checkpoint.")

    model = PaperSTGCN(
        Kt=Kt, Ks=Ks, n_his=n_his, n_vertex=n_vertex, gso=gso, droprate=0.0
    ).to(device)
    ...
    print("\n" + "=" * 78)
    print(f"Epoch {args.epoch} autoregressive test")
    print("=" * 78)

    print_metrics("Test:", metrics)

    metrics_path = selection_directory / (
        f"selected_checkpoint_epoch_" f"{args.epoch:03d}_metrics.csv"
    )

    selected_rows = save_metrics(metrics, metrics_path)

    final_metrics_path = result_directory / "final_metrics.csv"

    comparison_path = None

    if final_metrics_path.exists():
        final_frame = pd.read_csv(final_metrics_path)

        selected_by_minutes = {int(row["minutes"]): row for row in selected_rows}
            ...
            comparison_rows.append(
                {
                    ...
                    "selected_MAPE_percent": selected["MAPE_percent"],
                    "final_epoch_MAPE_percent": float(final_row["MAPE_percent"]),
                    "MAPE_difference_pp": selected["MAPE_percent"]
                    - float(final_row["MAPE_percent"]),
                    "selected_MAE": selected["MAE"],
                    "final_epoch_MAE": float(final_row["MAE"]),
                    "MAE_difference": selected["MAE"] - float(final_row["MAE"]),
                    "selected_RMSE": selected["RMSE"],
                    "final_epoch_RMSE": float(final_row["RMSE"]),
                    "RMSE_difference": selected["RMSE"] - float(final_row["RMSE"]),
                }
            )

        comparison_path = selection_directory / (
            f"selected_checkpoint_epoch_" f"{args.epoch:03d}_vs_final.csv"
        )
    ...
    summary_path = selection_directory / (
        f"selected_checkpoint_epoch_" f"{args.epoch:03d}_summary.json"
    )
