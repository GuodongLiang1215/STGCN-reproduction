...
if not HISTORY_PATH.exists():
    raise FileNotFoundError(
        f"没有找到训练历史：{HISTORY_PATH}"
    )

if not FINAL_METRICS_PATH.exists():
    raise FileNotFoundError(
        f"没有找到最终指标：{FINAL_METRICS_PATH}"
    )
...
reproduction_metrics = pd.read_csv(
    FINAL_METRICS_PATH
)

reproduction_metrics = (
    reproduction_metrics.rename(
        columns={
            "MAPE_percent":
...
comparison = reproduction_metrics.merge(
    paper_metrics,
    on="minutes",
    how="inner",
)
...
comparison["MAE_difference"] = (
    comparison["reproduction_MAE"]
    - comparison["paper_MAE"]
)

comparison["RMSE_difference"] = (
    comparison["reproduction_RMSE"]
...
best_validation_path = (
    ANALYSIS_DIRECTORY
    / "best_validation_epochs.csv"
)

best_validation.to_csv(
    best_validation_path,
    index=False,
)
...
plt.plot(
    history["epoch"],
    history["copy_l2_per_sample"],
    label="Copy-last-value baseline",
)
...
plot_validation_curves(
    metric_suffix="mae",
    ylabel="MAE",
    filename="validation_mae_curve.png",
)

plot_validation_curves(
    metric_suffix="rmse",
    ylabel="RMSE",
    filename="validation_rmse_curve.png",
)
...
def plot_paper_comparison(
    ...
    paper_values = comparison[
        paper_column
    ].to_numpy()

    reproduction_values = comparison[
        reproduction_column
    ].to_numpy()
...
generated_files = [
    ...
    / "learning_rate_curve.png",
    ANALYSIS_DIRECTORY
    / "validation_mape_curve.png",
    ANALYSIS_DIRECTORY
    / "validation_mae_curve.png",
    ANALYSIS_DIRECTORY
    / "validation_rmse_curve.png",
    ANALYSIS_DIRECTORY
    / "paper_mape_comparison.png",
    ANALYSIS_DIRECTORY
    / "paper_mae_comparison.png",
    ANALYSIS_DIRECTORY
    / "paper_rmse_comparison.png",
]
