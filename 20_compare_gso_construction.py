...
OUTPUT_DIRECTORY = (
    PROJECT_ROOT
    / "results"
    / "gso_construction_verification"
)


# =========================================================
# 2. Original TensorFlow STGCN scaled Laplacian
# =========================================================
...
# =========================================================
# 3. Current PyTorch utility GSO
# =========================================================

def current_utility_gso(
...
def main() -> None:
    ...
    if not OFFICIAL_ADJ_PATH.exists():
        raise FileNotFoundError(
            ...
            f"{OFFICIAL_ADJ_PATH}\n\n"
            "Run 18_prepare_official_adjacency.py first."
        )

    adjacency_sparse = sp.load_npz(
        OFFICIAL_ADJ_PATH
    ).tocsc()
    ...
    report = {
        "official_adjacency_path":
            str(OFFICIAL_ADJ_PATH),
        "adjacency_nonzero_count":
            int(adjacency_sparse.nnz),
        "isolated_node_count":
            int(isolated_mask.sum()),
        "isolated_node_indices":
            isolated_indices.tolist(),
        "original_largest_eigenvalue":
            original_lambda_max,
        "current_utility_warnings":
            current_warnings,
        "current_utility_gso":
            matrix_statistics(
                current_gso
            ),
        "original_tensorflow_gso":
            matrix_statistics(
                original_gso
            ),
    ...
    lines = [
        ...
        "=" * 62,
        "",
        f"Adjacency non-zero count: "
        f"{adjacency_sparse.nnz}",
        f"Isolated node count: "
        f"{isolated_mask.sum()}",
        f"Isolated node indices: "
        f"{isolated_indices.tolist()}",
        "",
        "Original TensorFlow construction:",
        f"  largest eigenvalue: "
        f"{original_lambda_max:.12g}",
        f"  GSO min: "
        f"{original_gso.min():.12g}",
        f"  GSO max: "
        f"{original_gso.max():.12g}",
        "",
        "Current PyTorch utility construction:",
        f"  GSO min: "
        f"{current_gso.min():.12g}",
        f"  GSO max: "
        f"{current_gso.max():.12g}",
        "",
        ...
        (
            "  mean abs difference: "
            f"{comparison['mean_abs_difference']:.12g}"
        ),
        (
            "  RMSE difference: "
            f"{comparison['rmse_difference']:.12g}"
        ),
        (
            "  exact equal: "
            f"{comparison['exact_equal']}"
        ),
        (
            "  allclose atol=1e-8: "
            f"{comparison['allclose_atol_1e-8']}"
        ),
        (
            "  allclose atol=1e-6: "
            f"{comparison['allclose_atol_1e-6']}"
        ),
        "",
        "Captured warnings:",
    ]
    ...
    print()
    print(
        "Adjacency non-zero count:",
        adjacency_sparse.nnz,
    )
    print(
        "Isolated node count:",
        int(isolated_mask.sum()),
    ...
    print()
    print(
        "Original largest eigenvalue:",
        original_lambda_max,
    )
    print(
        "Original GSO range:",
        (
            float(original_gso.min()),
            float(original_gso.max()),
        ),
    )
    print(
        "Current GSO range:",
        (
            float(current_gso.min()),
            float(current_gso.max()),
        ),
    )
    print()
    print(
        "Isolated-node diagonal "
        "(current):",
        report[
            "isolated_node_diagonal"
        ][
