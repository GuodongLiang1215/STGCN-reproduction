...
test_normalized = zscore.transform(
    test_data
).astype(np.float32)
...
with torch.no_grad():
    ...
    block2_output = model.st_blocks[1](
        block1_output
    )

    output_block_result = model.output(
        block2_output
    )

    full_output = model(x)

    prediction_normalized = full_output.view(
        1,
        n_vertex,
    )
...
prediction_original = zscore.inverse_transform(
    prediction_normalized.cpu().numpy()
)

target_original = zscore.inverse_transform(
    y.cpu().numpy()
)
