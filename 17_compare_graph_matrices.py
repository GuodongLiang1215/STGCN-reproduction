...
# =========================================================
# 3. Original STGCN graph preprocessing
# =========================================================
...
def original_stgcn_weight_matrix(
    ...
    unique_values = set(
        np.unique(source_matrix).tolist()
    )
...
# =========================================================
# 4. Download and locate official matrix
# =========================================================

def prepare_official_dataset() -> Path:
    """Download and extract the official PeMSD7 archive."""

    OFFICIAL_DATA_DIRECTORY.mkdir(
        parents=True,
        exist_ok=True,
    )

    existing_files = list(
        OFFICIAL_DATA_DIRECTORY.rglob(
            "PeMSD7_W_228.csv"
        )
    )
    ...
    if not OFFICIAL_ZIP_PATH.exists():
        print(
            "Downloading official PeMSD7 archive..."
        )
    ...
    if not matrix_files:
        raise FileNotFoundError(
            "The ZIP was extracted, but "
            "PeMSD7_W_228.csv was not found."
        )
...
def main() -> None:
    ...
    if not CURRENT_ADJ_PATH.exists():
        raise FileNotFoundError(
            f"Current adjacency matrix not found: "
            f"{CURRENT_ADJ_PATH}"
        )

    official_matrix_path = (
    ...
    if official_source.shape != (
        228,
        228,
    ):
        raise ValueError(
            "Unexpected official matrix shape: "
            f"{official_source.shape}"
        )

    # Reproduce original TensorFlow preprocessing
    official_weighted = (
        original_stgcn_weight_matrix(
            official_source,
            sigma2=0.1,
            epsilon=0.5,
        )
    ...
    matrices = {
        ...
        "official_source_csv":
            official_source,
        "official_after_original_weight_matrix":
            official_weighted,
        "official_after_original_weight_matrix_symmetrized":
            official_weighted_sym,
    }
    ...
    report_lines = [
        ...
        f"Current NPZ SHA256: "
        f"{sha256_file(CURRENT_ADJ_PATH)}",
        "",
        f"Official CSV: {official_matrix_path}",
        f"Official CSV SHA256: "
        f"{sha256_file(official_matrix_path)}",
        "",
        "Matrix statistics:",
    ]
        ...
        report_lines.extend(
            [
                ...
                (
                    ...
                    f"{row['max_abs_difference']}"
                ),
                (
                    f"mean_abs_difference = "
                    f"{row['mean_abs_difference']}"
                ),
                (
                    f"rmse_difference = "
                    f"{row['rmse_difference']}"
                ),
                (
                    f"exact_equal = "
                    f"{row['exact_equal']}"
                ...
                (
                    f"allclose_atol_1e-8 = "
                    f"{row['allclose_atol_1e-8']}"
                ),
                (
                    f"allclose_atol_1e-6 = "
                    f"{row['allclose_atol_1e-6']}"
                ),
                (
                    f"allclose_atol_1e-5 = "
                    f"{row['allclose_atol_1e-5']}"
                ),
            ]
        )
    ...
    for row in comparisons:
        ...
        print(
            "  allclose (1e-8):",
            row["allclose_atol_1e-8"],
        )
        print(
            "  allclose (1e-5):",
            row["allclose_atol_1e-5"],
        )
