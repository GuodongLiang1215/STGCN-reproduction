...
for seed, metrics_path in SEED_FILES.items():
    ...
    frame["seed"] = seed
    frame["selected_epoch"] = (
        SELECTED_EPOCHS[seed]
    )
    frame["metrics_path"] = str(
...
for minutes in (15, 30, 45):
    horizon_frame = per_seed[
        per_seed["minutes"] == minutes
    ]

    row = {
        "minutes": minutes,
        "n_seeds": len(horizon_frame),
    }
...
# =========================================================
# 4. Compare mean result with paper
# =========================================================

comparison = mean_std.merge(
    PAPER_RESULTS,
    on="minutes",
    how="left",
)
...
# =========================================================
# 6. Plot mean ± standard deviation
# =========================================================
...
def plot_metric(
    ...
    bars = plt.bar(
        ...
        yerr=stds,
        capsize=6,
        label="PyTorch reproduction (mean ± SD)",
    )
    ...
    plt.ylabel(ylabel)
    plt.title(
        f"STGCN Three-Seed Reproduction — {ylabel}"
    )
...
for _, row in comparison.iterrows():
    ...
    summary_lines.extend(
        [
            ...
            (
                "  MAE difference: "
                f"{row['MAE_mean_minus_paper']:+.6f}"
            ),
            (
                "  RMSE difference: "
                f"{row['RMSE_mean_minus_paper']:+.6f}"
            ),
        ]
    )
