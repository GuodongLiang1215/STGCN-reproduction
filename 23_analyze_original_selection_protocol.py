...
def get_arguments():
    ...
    parser.add_argument(
        ...
        help=(
            "Run directory under results/ and checkpoints/."
        ),
    )
...
def checkpoint_path_for_epoch(
    ...
    return (
        ...
        / f"paper_stgcn_epoch_{epoch:03d}.pt"
    )
...
def main():
    ...
    if missing_columns:
        raise ValueError(
            "Missing training-history columns: "
            f"{sorted(missing_columns)}"
        )

    # Original initialization:
    # [40, 1e5, 1e5] repeated for 3 horizons.
    running_minimum = np.asarray(
        [40.0, 1e5, 1e5] * 3,
        dtype=np.float64,
    )
    ...
    for _, row in history.iterrows():
        ...
        if improved_indices.size > 0:
            ...
            for local_index, metric_index in enumerate(
                improved_indices
            ):
                metric_name, _, _ = (
    ...
    if triggers.empty:
        raise RuntimeError(
            "No validation improvements were detected."
        )

    last_trigger_epoch = int(
        triggers.iloc[-1]["epoch"]
    )
    ...
    triggers_path = (
        output_directory
        / "original_protocol_trigger_epochs.csv"
    )

    improvements_path = (
        output_directory
        / "original_protocol_metric_improvements.csv"
    )
    ...
    summary_path = (
        output_directory
        / "selection_protocol_summary.txt"
    )
    ...
    summary_lines = [
        ...
        (
            "Last-trigger checkpoint exists: "
            f"{last_trigger_checkpoint.exists()}"
        ),
        (
            "Final checkpoint exists: "
            f"{final_checkpoint.exists()}"
        ),
        "",
    ...
    print("=" * 78)
    print(
        "Original STGCN selection-protocol audit"
    )
    print("=" * 78)
    ...
    print(
        f"Last trigger epoch: "
        f"{last_trigger_epoch}"
    )
    print(
        "Last-trigger checkpoint exists:",
        last_trigger_checkpoint.exists(),
    )
    ...
    if last_trigger_epoch == final_epoch:
        ...
        print(
            "Evaluate the last-trigger checkpoint "
