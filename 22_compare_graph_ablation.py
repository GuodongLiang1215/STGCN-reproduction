...
# =========================================================
# 3. Add paper differences
# =========================================================

comparison = long_results.merge(
    PAPER_RESULTS,
    on="minutes",
    how="left",
)
...
comparison["MAPE_difference_pp"] = (
    comparison["MAPE_percent"]
    - comparison["paper_MAPE_percent"]
)

comparison["MAE_difference"] = (
    comparison["MAE"]
    - comparison["paper_MAE"]
)

comparison["RMSE_difference"] = (
    comparison["RMSE"]
    - comparison["paper_RMSE"]
)

comparison["MAPE_abs_difference_pp"] = (
    comparison["MAPE_difference_pp"].abs()
)

comparison["MAE_abs_difference"] = (
    comparison["MAE_difference"].abs()
)

comparison["RMSE_abs_difference"] = (
    comparison["RMSE_difference"].abs()
)


# =========================================================
# 4. Pairwise controlled changes
# =========================================================
...
for minutes in (15, 30, 45):
    for metric in (
        ...
        baseline_value = float(
            pivot.loc[
                minutes,
                (metric, current_current),
            ]
        )

        official_graph_value = float(
            pivot.loc[
                minutes,
                (metric, official_current),
            ]
        )

        original_gso_value = float(
            pivot.loc[
                minutes,
                (metric, official_original),
            ]
        )
        ...
        pairwise_rows.append(
            {
                "minutes": minutes,
                "metric": metric,
                "A_current_graph_current_GSO":
                    baseline_value,
                "B_official_graph_current_GSO":
                    official_graph_value,
                "C_official_graph_original_GSO":
                    original_gso_value,
                "B_minus_A_graph_effect":
                    (
                        official_graph_value
                        - baseline_value
                    ),
                "C_minus_B_gso_effect":
                    (
                        original_gso_value
                        - official_graph_value
                    ),
                "C_minus_A_total_effect":
                    (
                        original_gso_value
                        - baseline_value
                    ),
...
# =========================================================
# 5. Best configuration per metric and horizon
# =========================================================

best_rows = []

for minutes in (15, 30, 45):
    horizon_frame = long_results[
        long_results["minutes"] == minutes
    ]
    ...
    for metric in (
        ...
        best_index = horizon_frame[
            metric
        ].idxmin()

        best_row = long_results.loc[
            best_index
        ]
...
long_path = (
    OUTPUT_DIRECTORY
    / "graph_ablation_long.csv"
)

comparison_path = (
    OUTPUT_DIRECTORY
    / "graph_ablation_vs_paper.csv"
)

pairwise_path = (
    OUTPUT_DIRECTORY
    / "pairwise_controlled_effects.csv"
)
...
def plot_metric(
    ...
    plt.ylabel(ylabel)
    plt.title(
        f"STGCN Graph-Pipeline Ablation — {ylabel}"
    )
    plt.grid(
        axis="y",
        alpha=0.3,
...
for minutes in (15, 30, 45):
    ...
    horizon_rows = pairwise_comparison[
        pairwise_comparison["minutes"]
        == minutes
    ]
    ...
    for _, row in horizon_rows.iterrows():
        ...
        summary_lines.append(
            "    graph effect (B-A) = "
            f"{row['B_minus_A_graph_effect']:+.6f}"
        )

        summary_lines.append(
            "    GSO effect (C-B) = "
            f"{row['C_minus_B_gso_effect']:+.6f}"
        )

        summary_lines.append(
            "    total effect (C-A) = "
            f"{row['C_minus_A_total_effect']:+.6f}"
        )
...
for path in (
    ...
    summary_path,
    OUTPUT_DIRECTORY
    / "graph_ablation_mape.png",
    OUTPUT_DIRECTORY
    / "graph_ablation_mae.png",
    OUTPUT_DIRECTORY
    / "graph_ablation_rmse.png",
):
    print(path)
